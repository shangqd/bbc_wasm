...
namespace wasm_constraints = eosio::chain::wasm_constraints;
   ...
   void shang_test2(const char* code_bytes, size_t code_size, std::vector<uint8_t> initial_memory)
   {
      ...
      //eosio::chain::apply_context *context = nullptr;
      ...
      EOS_ASSERT( res1.result == interp::Result::Ok, wasm_execution_error, "wabt start function failure (${s})", ("s", ResultToString(res1.result)) );
      ...
      //EOS_ASSERT( res1.result == interp::Result::Ok, wasm_execution_error, "wabt execution failure (${s})", ("s", ResultToString(res1.result)) );
   }
...
class wabt_instantiated_module : public wasm_instantiated_module_interface {
   public:
      ...
      void apply(apply_context& context) override {
         ...
         std::cout << uint64_t(context.get_receiver()) << std::endl;
         std::cout << uint64_t(context.get_action().account) << std::endl;
         std::cout << uint64_t(context.get_action().name) << std::endl;
